...
class BaseTestCase(TestCase):
    ...
    def setUp(self):
        ...
        db.session.add(Users(username="test", password="$2y$12$lcYrKeAkB5b7dYRep1Yj6etbkmfqGXONkWrFVbs6rVcIOm6KN.1lC",
                             admin_role=True))
        db.session.add(Users(username="testNotAdmin", password="$2y$12$kaTE2iYpH2jemQEGGhkcP.t1af04EvO/.JdZ5/QjibfpEdvz7Orra ",
                             admin_role=False))
        db.session.commit()
...
class FlaskTestCase(BaseTestCase):
    ...
    def test_login_is_required_home(self):
        response = self.client.get('/room', follow_redirects=True)
        self.assertTrue(b'Unauthorized' in response.data)

    def test_login_is_required_register(self):
        response = self.client.get('/register', follow_redirects=True)
        self.assertTrue(b'Unauthorized' in response.data)

    def test_login_is_required_viewAccounts(self):
        response = self.client.get('/viewAccounts', follow_redirects=True)
        self.assertTrue(b'Unauthorized' in response.data)

    def test_login_is_required_changePassword(self):
        response = self.client.get('/changePassword', follow_redirects=True)
        self.assertTrue(b'Unauthorized' in response.data)
